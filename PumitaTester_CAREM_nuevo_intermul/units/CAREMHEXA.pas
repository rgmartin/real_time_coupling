...
//  ArchivoSalidaAbierto := false;
  ...
  for i:=1 to 6 do ArchivoIntMultiple[i] := '';              InterpolacionMultipleInicializada := false;
...
//**************************************************************************/
 ...
 begin
  ...
  if DT = 0.0 then
  begin
    FNORM := POWER/ACC/(1.0 + FracPotResidual);
    for canal:=1 to NCANALES do
    for trozo:=1 to NTROZOS do begin
      for grupo:=1 to ng do
...
//**************************************************************************/
 ...
 var
   nreg,iz, iteracion, NNN,  NP, MaxIter, grupo, grupo1, nborde, nvecino: integer;
   EPS0, EX, EPSKE: real;
 ...
 begin
  ...
  if ImprimirIteraciones then begin
    Writeln(F);
